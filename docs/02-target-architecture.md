...
Фиксируем v1 (lean): **server-rendered web UI внутри Ktor** (Kotlin templates через `kotlinx.html` / Ktor HTML DSL).

Причина (предпринимательский компромисс): это позволяет сделать рабочую админку силами backend-инженера без отдельного frontend build-пайплайна и без расширения команды.

Деплой v1 (минимум систем):
- UI отдаётся тем же Kotlin-монолитом (один домен, один deployable).
